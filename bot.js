...
const botConfig = require('./res/devconfig.json');
...
app.post("/moderation", (req, res) => {
	...
	if (req.body.type === 'banAppeal') {
		...
		client.channels.cache.get(banAppeal.complaint).send(embed);
	}
	...
	app.post('/twitch', (req, res) => {
		...
		getTwitchToken().then(twitchToken => {
			...
			if (!twitchVerifySignature()) {
				...
				do {
					fetch('https://api.twitch.tv/helix/streams?user_login=monty_lion', {
						method: 'get',
						headers: {
							'Content-Type': 'application/json',
							'Client-ID': 'muexic89yevvg6vry4rdlv7byc4656',
